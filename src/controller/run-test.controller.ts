...
export class RunTest {
  ...
  async executeDockerCall(
    ...
    return new Promise((resolve) => {
      ...
      const command = `docker run --rm -v ${cwd}/user_assessments/${userCodeFileName}:/app/script-solidity/contracts/Lock.sol solidity sh -c "npx hardhat test"`;
  ...
  async handle(@Body() body: any) {
    const { solidityCode } = body;

    const fileName = this.createTmpFile(solidityCode);
